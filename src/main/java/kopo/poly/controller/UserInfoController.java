...
public class UserInfoController {
    ...
    public String insertUserInfo(HttpServletRequest request, ModelMap modelMap) throws Exception {
        ...
        try {
            ...
            if (res == 1) {
                msg = "회원가입되었습니다.";
                url = "/user/login";
                //추후 회원가입 입력화면에서 ajax를 활용해서 아이디 즁복, 이메일 중복을 체크하길 바람
            } else {
    ...
    public String loginProc(HttpServletRequest request, ModelMap model, HttpSession session) {
        ...
        try {
            ...
            //비밀번호는 절대로 복호화 되지 않도록 해서 알고리즘으로 암호화함
            pDTO.setPassword(EncryptUtil.encHashSHA256(password));
             ...
             *     따라서 과도한 세셩은 톰켓의 메모리 부하를 발생시켜 서버가 다운되는 현상이 있을 수 있기 때문에,
             *     최소한으로 사용하는 것을 권장한다.
             *
             * 스프링에서 세션을 사용하기 위해서는 함수명의 파라미터에 HttpSession session 존재해야 한다.
             * 세션은 톰켓의 메모리에 저장되기 때문에 url마다 전달하는게 필요하지 얺고,
             * 그냥 메모리에서 부르면 되기 때문에 화면, conttroller에서 쉽게 불러서 볼수 있다.
             * */
